...
import { UserFollowRequest, UserPaginationResponse, UserRequest, UserResponse } from '../models/User';
...
export class UserService {
  ...
  follow(user_id: number, followed_id: number){
    return new Promise((accept, reject) => {

      this._apiService.post<UserFollowRequest,boolean>(APIs.user.follow+user_id,{followee_id: followed_id}).subscribe((data) => {
        if(data){
          accept(data);
        }else{
          reject(data);
  ...
  unfollow(user_id: number, followed_id: number){
    return new Promise((accept, reject) => {

      this._apiService.post<UserFollowRequest,boolean>(APIs.user.unfollow+user_id,{followee_id: followed_id}).subscribe((data) => {
        if(data){
          accept(data);
        }else{
          reject(data);
